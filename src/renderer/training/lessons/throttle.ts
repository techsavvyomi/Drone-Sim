...
export const throttleLesson: Lesson = {
  ...
  practice: {
    prompt: 'Arm, take off, then down to about a metre, hold it, and back up',
  ...
  // Every limit here carries the take-off now: arming and the climb to the
  // hover cost about eight seconds before the drill's own clock means anything.
  stars: [
    {
      stars: 3,
      text: 'Off the pad, down, held and back up in 30s, smoothly, nothing touched',
      test: ({ touches, timeSec, collisions, smoothness }) =>
        collisions === 0 && touches === 0 && smoothness >= 0.5 && timeSec <= 30,
    },
    {
      stars: 2,
      text: 'Off the pad, down, held and back up in 50s',
      test: ({ timeSec, collisions, smoothness }) =>
        collisions === 0 && smoothness >= 0.25 && timeSec <= 50,
    },
  ],
